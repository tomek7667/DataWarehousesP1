...
def insertEquipment(eq):
	return f"INSERT INTO equipments (name, brandName, size, isRental, isOnShelf, price, inStock) VALUES (" + "'" + eq.name + "', '" + eq.brandName + "', '" + str(eq.size) + "', " + str(eq.isRental) + ", " + str(eq.isOnShelf) + ", " + str(eq.price) + ", " + str(eq.inStock) + ");"


def insertCashier(ca):
	return f"INSERT INTO cashiers (pesel, name, surname, dateOfEmployment) VALUES ('{ca.pesel}', " + "'" + str(ca.name) + "', '" + str(ca.surname) + "', '" + str(ca.dateOfEmployment) + "');"


def insertBill(bill):
	return f"INSERT INTO bills (cashierID, amount, paymentDatetime) VALUES ({bill.cashierID}, {bill.amount}, " + "'" + str(bill.paymentDatetime) + "');"
...
equipmentsS1 = [Equipment(i, 1) for i in range(equipmentN)] #pytanie jak z cenami i czy nie lepiej zrobic jedna tablice sprzetu po prostu
...
def main():
	...
	questionnaireExcel = pd.DataFrame(columns=('fulfillment_date', 'price', 'name', 'brand_name', 'comfort', 'rentPrice', 'visage', 'overall', 'equipment_general_rating'))

	billID = 1 #Kolejne indeksy bill
	rentalID = 1 #Kolejne indeksy rentalID
	season = 1
	for day in period.values[40:264]:
		...
		isThunder = random.randint(0, 100) > 98
		isBadWeather = isThunder or random.randint(0, 100) > 90
		...
		if isThunder:
			nofp = random.randint(0, 8)
		elif isBadWeather:
			nofp = random.randint(40, 107)
		else:
			nofp = random.randint(120, 200)

		for i in range(nofp):
			howManyEquipments = random.randint(1, 5)
			equipmentsToRent = []
			equipmentsToBuy = []
			shopAmount = 0
			rentAmount = 0
...
# Tworzenie class dla sprzedaży
			...
			if len(equipmentsToRent) > 0:
				...
				for eq in equipmentsToRent:
					...
					specificrental = SpecificRental(rentalID, eq.ID, startDatetime, plannedEndDatetime, actualEndDatetime, howManyMinutesLate)
					...
					if random.randint(0,100) > (100-chanceForQuestionnaire):
						...
						qData = pd.DataFrame([[q.fulfillmentDate, q.price, q.name, q.brandName, q.comfort, q.rentPrice, q.visage, q.overall, q.eqGeneralRating]], columns=('fulfillment_date', 'price', 'name', 'brand_name', 'comfort', 'rentPrice', 'visage', 'overall', 'equipment_general_rating'))
